...
const AssetDetails = () => {
  ...
  const { nftCurrency, buyNft, isLoadingNFT, getUser, imagePaths } = useContext(NFTContext);
  ...
  useEffect(async () => {
    ...
    setNft(router.query);
    const { 0: seller } = await getUser(router.query.seller);
    setSellerId(seller.userId);
    setIsLoading(false);
  }, [router.isReady]);
    ...
    <div className="relative flex justify-center md:flex-col min-h-screen">
      <div className="relative flex-1 flexCenter sm:px-4 p-12 border-r md:border-r-0 md:border-b dark:border-nft-black-1 border-nft-gray-1">
        <div className="relative w-557 minmd:w-2/3 minmd:h-2/3 sm:w-full sm:h-300 h-557 ">
          {!nft.image.includes('https://oaidalleapiprodscus') && <Image src={nft.image || images[`nft${nft.i}`]} objectFit="cover" className=" rounded-xl shadow-lg" layout="fill" />}
        </div>
      </div>
        ...
        <div className="mt-10">
          ...
          <div className="flex flex-row items-center mt-3">
            <div className="relative w-12 h-12 minlg:w-20 minlg:h-20 mr-2">
              <Image src={imagePaths[sellerId]} objectFit="cover" className="rounded-full" />
            </div>
